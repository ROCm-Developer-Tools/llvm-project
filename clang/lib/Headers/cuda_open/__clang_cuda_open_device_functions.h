...
//
//===----------------------------------------------------------------------===//
#ifndef __CLANG_CUDA_OPEN_DEVICE_FUNCTIONS_H__
#define __CLANG_CUDA_OPEN_DEVICE_FUNCTIONS_H__
#ifndef __CUDA__
#error "This file is for CUDA compilation only."
#endif

#ifdef __MAKE_DEVICE_FUNCTIONS_EXTERN__
//  When calling for openmp device functions, we need extern because that is
//  how many standard headers define functions
#define __DEVICE__ __device__ __forceinline__
#else
//  When building a library, we need these functions to be static
...
#if __AMDGCN__ > 800
__DEVICE__ long long __clock64(void) {
  return __builtin_amdgcn_s_memrealtime();
}
__DEVICE__ long long __clock(void) {
  return (uint)__builtin_amdgcn_s_memrealtime();
}
#else
__DEVICE__ long long __llvm_amdgcn_s_memrealtime();
__DEVICE__ long long __clock64(void) { return __llvm_amdgcn_s_memrealtime(); }
__DEVICE__ long long __clock(void) {
  return (uint)__llvm_amdgcn_s_memrealtime();
}
#endif
#endif
