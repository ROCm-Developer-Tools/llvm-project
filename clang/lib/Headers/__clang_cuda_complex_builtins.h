...
// One successful way to match cexp generated by clang codegen
// of code like this  "res=exp(inp);" is to use _Complex double
// for both arg and return types
_Complex double cexp(_Complex double _arg_cd){
  __2f64 _arg_copy_2f64 ;
  memcpy(&_arg_copy_2f64, &_arg_cd, sizeof(__2f64));
  __2f64 _result_2f64 = __ocml_cexp_f64(_arg_copy_2f64);
  _Complex double res_cd;
  memcpy(&res_cd, &_result_2f64, sizeof(__2f64));
  return res_cd;
}
_Complex float cexpf(_Complex float _arg_cf){
  __2f32 in_f ;
  memcpy(&in_f, &_arg_cf, sizeof(__2f32));
  __2f32 res_f = __ocml_cexp_f32(in_f);
  _Complex float res_cf;
  memcpy(&res_cf, &res_f, sizeof(__2f32));
  return res_cf;
}
