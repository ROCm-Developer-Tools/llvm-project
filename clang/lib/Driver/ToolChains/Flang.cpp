...
void Flang::addOffloadOptions(const Compilation &C, const JobAction &JA,
  ...
  if (IsOpenMPDevice) {
    ...
    // the host, so the meaningful device declarations can be identified. Also,
  ...
  if ((D.isSaveTempsEnabled() || IsOpenMPHost) &&
      !(D.embedBitcodeInObject() && !IsUsingLTO) && isa<CompileJobAction>(JA))
...
void Flang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (isa<PreprocessJobAction>(JA)) {
    ...
    if (JA.getType() == types::TY_Nothing) {
      ...
      if (Triple.isAMDGCN() && JA.isDeviceOffloading(Action::OFK_OpenMP) &&
