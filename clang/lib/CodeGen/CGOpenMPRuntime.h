...
public:
class RegisterDeclareTargetInterfaceClang
    : public llvm::RegisterDeclareTargetInterface {
public:
  RegisterDeclareTargetInterfaceClang(const VarDecl *Decl,
                                      CodeGenModule &CGM)
      : Decl(Decl), CGM(CGM) {
    captureKind = mapCaptureClause(Decl);
    deviceType = mapDeviceClause(Decl);
  }
...
private:
  ...
  mapDeviceClause(const VarDecl* VD) {
  ...
  mapCaptureClause(const VarDecl* VD) {
    ...
    case OMPDeclareTargetDeclAttr::MapTypeTy::MT_Enter:
      return DeclareTargetCaptureClauseKind::Enter;
      break;
