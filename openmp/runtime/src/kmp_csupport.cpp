...
void __kmpc_serialized_parallel(ident_t *loc, kmp_int32 global_tid) {
  ...
  __kmp_assert_valid_gtid(global_tid);
#if OMPT_SUPPORT
  // This function can be called directly from the user application code
  // or can be called from a runtime function. If the enter frame is set,
  // that means that this function is called from a runtime function
  // (caller set the runtime frame).
  ompt_frame_t *parent_frame;
  parent_frame = &OMPT_CUR_TASK_INFO(__kmp_threads[global_tid])->frame;
...
#if OMPT_SUPPORT
  // If this function is called by the runtime frame,
  // the exit_frame is going to be set as usual.
  if (!runtime_caller) {
    // Otherwise, do the following.
    // (This should only happen when parallel if(0) is used inside the test case
    // compiled by using clang.)

    // Implicit task wrapper (.omp_outlined..) is called directly from the
    // user code immediately after this function returns. If I understood well,
    // exit_frame is going to be set inside the wrapper .omp_outlined.. and will
    // point to it (or inside .omp_outlined._debug__ if -g flag is
    // used and will point to .omp_outlined._debug__).
    // Corresponding exit_flags will be set to indicate that the frame
    // belongs to the runtime, even though it is generated by the clang and obviously
    // belongs to the user application code. In order to prevent this,
    // store exit_frame here and set the right flags.
