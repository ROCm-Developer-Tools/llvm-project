...
void AMDGPU::fillAMDGPUFeatureMap(StringRef CPU, const Triple &T,
                                  StringMap<bool> &Features) {
  // XXX - What does the member GPU mean if device name string passed here?
  if (T.isAMDGCN()) {
    switch (parseArchAMDGCN(CPU)) {
    case GK_GFX1103:
    case GK_GFX1102:
    case GK_GFX1101:
    case GK_GFX1100:
...
static bool isWave32Capable(StringRef CPU, const Triple &T) {
...
bool AMDGPU::insertWaveSizeFeature(StringRef CPU, const Triple &T,
                                   StringMap<bool> &Features,
                                   StringRef ErrorMsg) {
  bool IsWave32Capable = isWave32Capable(CPU, T);
